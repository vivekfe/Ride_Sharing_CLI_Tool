...
class SalaryCalculator(object):
    ...
    def ride_finished(ride_date: str, driver_id: int, ride_id: int, time_taken: int, distance: int) -> None:
        try:
            ...
            cur.execute("INSERT INTO RIDE_DETAILS (ride_date, driver_id, ride_id, time_taken, distance) VALUES (?, ?, ?, ?, ?)", (ride_date, driver_id, ride_id, time_taken, distance))
    ...
    def rating_submitted(driver_id, ride_id, rating) -> None:
        try:
            ...
            db_connection = db_manager.connect_to_db()
            cur = db_connection.cursor()
            cur.execute("INSERT INTO DRIVER_RATING (driver_id, ride_id, rating) VALUES (?,?,?)",  (driver_id, ride_id, rating) )
            db_connection.commit()
            cur.close()
            pretty_print('Inserted a new record in DB')
        except sqlite3.Error as e:
    ...
    @staticmethod
    def calculate_driver_payment(driver_id: int, period_start, period_end) -> float:
        try:
            db_manager = DatabaseManager(driver_storage)
            db_connection = db_manager.connect_to_db()
            cur = db_connection.cursor()
            ...
            rows= None
            cur.execute("SELECT SUM(distance) AS DISTANCE,SUM(time_taken) AS TIME_TAKEN FROM RIDE_DETAILS WHERE driver_id= (?) AND ride_date >=(?) AND ride_date<=(?)", (driver_id, period_start, period_end))
            rows = cur.fetchall()
            ...
            cur.close()
            db_connection.close()
            pretty_print(f"Calculate driver payment for Driver ID: {driver_id} between {period_start} and {period_end} is USD {total_payment}\n"+"/"*60)
